...
- 📱 **二维码远程录入**：PC 端弹出二维码 → 朋友手机扫码 → 在自己的手机上填表（头像 / 姓名 / ID / 需求精灵）→ 提交后实时同步到 PC。基于 WebRTC + PeerJS 公共信令，数据全程仅在两端浏览器间传输，不写入任何服务器。
...
- [PeerJS](https://peerjs.com/) + [qrcode](https://github.com/soldair/node-qrcode)：二维码远程录入的 WebRTC 通道与二维码渲染
